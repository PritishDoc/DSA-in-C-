    ...
    int robotSim(vector<int>& commands, vector<vector<int>>& obstacles) {
        ...
        for (int cmd : commands) {
            if (cmd == -2) {
                ...
                directionIndex = (directionIndex + 3) % 4; // equivalent to turning left
            } else if (cmd == -1) {
                // Turn right 90 degrees
                directionIndex = (directionIndex + 1) % 4; // equivalent to turning right
            } else {
                ...
                for (int i = 0; i < cmd; ++i) {
                    ...
                    if (obstacleSet.find(to_string(nextX) + "," + to_string(nextY)) == obstacleSet.end()) {
                        ...
                        // Update max distance squared from the origin
